...
class DispFreq(QtGui.QWidget):
	...
	def __init__(self):
		...
		self.timer_acquire = QtCore.QTimer()
		QtCore.QObject.connect(self.timer_update, QtCore.SIGNAL("timeout()"), self.update)
		QtCore.QObject.connect(self.timer_acquire, QtCore.SIGNAL("timeout()"), self.acquire)
		
		self.acquire()
		
		self.timer_update.start(1600)
		self.timer_acquire.start(2000)
		self.show()
	...
	def acquire(self):
		...
		self.fft2 = fft2[range(n2/2)]
		self.frq2 = frq2
		self.time_list2 = time_list2
		self.measures2 = measures2
